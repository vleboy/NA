...
export default {
  ...
  data () {
    return {
      ...
      parentMix: '', // 上级洗码比
      nowType: gameType('naArcade'), // 当前输赢报表游戏类型
    }
  },
  activated: function () {
    this.$store.commit({
                    ...
                    item.map(item=> {
                      data.map(side =>{
                        if (item.userName == side.userName) {
                          ...
                          item.gameList.filter(mix => {return mix.code == this.nowType}).length == 0 ? item.nowBouns = side.betAmount * this.parentMix : item.nowBouns = side.mixAmount * item.gameList.filter(mix => {return mix.code == this.nowType})[0].mix / 100
                          item.nowallBet = item.nowBouns + side.winloseAmount
