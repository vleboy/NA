...
export default {
  ...
  data () {
    return {
      ...
      parentMix: '', // 上级洗码比
      nowType: gameType('naVedio'), // 当前输赢报表游戏类型
    }
  },
  activated: function () {
    this.$store.commit({
      ...
      let result1 = invoke({
        ...
        if (err) {
          ...
          if (!user.gameList) {
            ...
            if (!user.gameList.filter(mix => {return mix.code == this.nowType}).length) {
              // 非代理管理员但没有代理此游戏  但有数据
              this.parentMix = 0
            } else {
              // 有此游戏且有数据
              ...
              if (data) {
                data.map(side =>{
                  if (user.userId == side.userId) {
                    ...
                    user.nowallBet = user.nowBouns + user.winloseAmount
                    user.winloseRate = user.nowallBet / user.betAmount
