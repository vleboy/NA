  ...
  methods: {
    ...
    async beginGame (gameType, sid, ttgGameType, ttgGameId, ttgGameName) {
      try {
        ...
        if (data.code === 0) {
          window.location.href = `http://webpage.na77.org/ttg.html?playerHandle=${data.token}&userId=${datas.userId}&gameId=${datas.sid}&ttgGameType=${ttgGameType}&ttgGameId=${ttgGameId}&ttgGameName=${ttgGameName}&parent=${datas.parent}&userName=${datas.userName}`
          this.loading = false
        } else {
          this.loading = false
          alert(data.msg)
