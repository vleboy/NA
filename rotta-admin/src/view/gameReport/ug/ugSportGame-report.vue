<template>
  <div class="ugSportGame-report" @click="getTimer">
    <div class="nowUserlist">
      ...
      <el-table :data="ugSportNowlist" stripe>
    ...
    <div class="childlist" v-if="loginRole != '100'">
      ...
      <el-table :data="ugSportNowchild" stripe>
        <el-table-column label="序号" align="left" width="75" type="index">
        </el-table-column>
        <el-table-column label="类型" prop="role" align="left" :formatter="userType">
        </el-table-column>
    ...
    <div class="childlist" v-for="item of ugSportRenderChild">
    ...
    <div class="playerlist">
      ...
      <el-table :data="ugSportPlayer" stripe>
...
export default {
  ...
  data () {
    return {
      ...
      nowType: gameType('ugSport'), // 当前报表游戏type
    ...
    getLoginSelf () {
      ...
      let result1 = invoke({
        ...
        if (err) {
          ...
          if (user.gamelist && user.gameList.filter(game => {return game.code == gameType('ugSport')}).length == 0) {
            this.$message({
              message: '您尚未购买此游戏',
              type: 'warning'
            })
          ...
          user.submit = 0
          user.winloseRate = 0
          localStorage.loginRole == 1 ? user.rate = 100 : user.rate = user.gameList.filter(game => {return game.code == gameType('ugSport')})[0].rate
          this.nowList = user
        }
      })
    }, // 获取登陆用户报表基本信息
          ...
          if (err) {
            ...
            child.map(item => {
              item.gameList.map(allGame => {
                allGame.code == gameType('ugSport') ? item.rate = allGame.rate : ''
              })
          ...
          if (err) {
            ...
            data.map(user => {
              user.gameList.map(game => {
                game.code == gameType('ugSport') ? user.rate == game.rate : ''
              })
            })
...
<style scpoed>
.ugSportGame-report .clearFix:after {clear:both;content:'.';display:block;width: 0;height: 0;visibility:hidden;}
.ugSportGame-report .input{width: 25rem}
.ugSportGame-report .page{padding-bottom: 2rem;text-align: right;margin-right: 1%;margin-top: 0.5rem;margin-top: 2rem}
.ugSportGame-report .title{font-size: 1.5rem;margin: 0 0 0.5rem 0;font-weight: 600;display: inline-block}
.ugSportGame-report .nowUserlist,
.ugSportGame-report .childlist,
.ugSportGame-report .playerlist{width: 99%;margin: 2rem auto}
.ugSportGame-report .fontUrl{cursor: pointer;color: #20a0ff}
.ugSportGame-report .fontUrl:hover{text-decoration: underline;}
