s<template>
  <div class="naLiveGame-report" @click="getTimer">
    ...
    <div class="playerlist">
      ...
      <el-table :data="naLivePlayer" stripe>
...
export default {
  ...
  data () {
    return {
      ...
      nowType: gameType('naLive'), // 当前报表游戏type
    ...
    getLoginSelf () {
      ...
      let result1 = invoke({
        ...
        if (err) {
          ...
          if (user.gameList && user.gameList.filter(game => {return game.code == gameType('naLive')}).length == 0) {
            this.$message({
              message: '您尚未购买此游戏',
              type: 'warning'
            })
          ...
          user.submit = 0
          user.winloseRate = 0
          localStorage.loginRole == 1 ? user.rate = 100 : user.rate = user.gameList.filter(game => {return game.code == gameType('naLive')})[0].rate
          ...
          if (err) {
            ...
            child.map(item => {
              item.gameList.map(allGame => {
                allGame.code == gameType('naLive') ? item.rate = allGame.rate : ''
              })
