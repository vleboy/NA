<template>
  <div class="mgVedioGame-report" @click="getTimer">
...
export default {
  ...
  data () {
    return {
      ...
      nowType: gameType('mgVedio'), // 当前报表游戏type
    ...
    getLoginSelf () {
      ...
      let result1 = invoke({
        ...
        if (err) {
          ...
          if (user.gameList && user.gameList.filter(game => {return game.code == gameType('mgVedio')}).length == 0) {
            this.$message({
              message: '您尚未购买此游戏',
              type: 'warning'
            })
          ...
          user.submit = 0
          user.winloseRate = 0
          localStorage.loginRole == 1 ? user.rate = 100 : user.rate = user.gameList.filter(game => {return game.code == gameType('mgVedio')})[0].rate
          this.nowList = user
        }
      })
    }, // 获取登陆用户报表基本信息
          ...
          if (err) {
            ...
            child.map(item => {
              item.gameList.map(allGame => {
                allGame.code == gameType('mgVedio') ? item.rate = allGame.rate : ''
              })
