<template>
  <div class="ttgVedioGame-report" @click="getTimer">
    ...
    <div class="childlist" v-for="item of ttgRenderChild">
...
export default {
  ...
  data () {
    return {
      ...
      nowType: gameType('ttgVedio'), // 当前报表游戏type
    ...
    getLoginSelf () {
      ...
      let result1 = invoke({
        ...
        if (err) {
          ...
          if (user.gamelist && user.gameList.filter(game => {return game.code == gameType('ttgVedio')}).length == 0) {
            this.$message({
              message: '您尚未购买此游戏',
              type: 'warning'
            })
          ...
          user.submit = 0
          user.winloseRate = 0
          localStorage.loginRole == 1 ? user.rate = 100 : user.rate = user.gameList.filter(game => {return game.code == gameType('ttgVedio')})[0].rate
          this.nowList = user
        }
      })
    }, // 获取登陆用户报表基本信息
          ...
          if (err) {
            ...
            child.map(item => {
              item.gameList.map(allGame => {
                allGame.code == gameType('ttgVedio') ? item.rate = allGame.rate : ''
              })
          ...
          if (err) {
            ...
            data.map(user => {
              user.gameList.map(game => {
                game.code == gameType('ttgVedio') ? user.rate == game.rate : ''
              })
            })
