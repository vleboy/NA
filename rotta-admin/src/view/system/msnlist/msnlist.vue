<template>
  <div class="msn">
    <div class="search">
        <el-input placeholder="请输入需要查询的线路号" class="input" v-model="searchMsn"></el-input><el-button type="primary" @click="searchMSNList">搜索</el-button><el-button type="primary" @click="resetList">重置</el-button>
    </div>
...
export default {
  ...
  methods: {
    searchMSNList () {
      if (!this.searchMsn) {
      ...
      } else if (isNaN(this.searchMsn)) {
        this.$message({
          message: '请输入正确的线路号',
          type: 'error'
        })
      } else if (Number(this.searchMsn) < 0 || Number(this.searchMsn) > 999) {
        this.$message({
          message: '线路号范围为1 ~ 999 间任意数字',
          type: 'error'
        })
      } else {
        this.$store.commit({
        ...
        this.$store.commit('searchMsnlist')
      }
    },
    resetList () {
      this.searchMsn = ''
...
.msn .page {padding-bottom: 2rem;text-align: right;margin-right: 1%;margin-top: 0.5rem;margin-top: 2rem}
/**/
.green{color: #00CC00}
.red{color: #ff3300}
